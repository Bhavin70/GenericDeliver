                ...
                if (tblInvoiceRptTODT != null)
                {
                    ...
                    while (tblInvoiceRptTODT.Read())
                    {
                        ...
                        for (int i = 0; i < tblInvoiceRptTODT.FieldCount; i++)
                        {
                            ...
                            //tblInvoiceRptTONew.OwnerPersonFirstAndLastName = (tblInvoiceRptTONew.OwnerPersonFirstName +"  "+  tblInvoiceRptTONew.OwnerPersonLastName);
                        }
        ...
        public List<TblInvoiceRptTO> SelectInvoiceExportList(DateTime frmDt, DateTime toDt, int isConfirm, int fromOrgId)
        {
            ...
            try
            {
                ...
                selectQuery = "select distinct  tblBookings.bookingRate,invoice.invFromOrgId,invoice.idInvoice,invoice.statusDate,invoice.invoiceDate,invoice.invoiceNo," +
                    ...
                    " igstAmt, grandTotal, vehicleNo, invoice.createdOn, isConfirmed, statusId, narration,invFromOrgId from tempInvoice invoice  LEFT JOIN tblOrganization org on org.idOrganization = invoice.distributorOrgId LEFT JOIN tblPerson person on person.idPerson = org.firstOwnerPersonId INNER JOIN tempInvoiceAddress invoiceAdd on invoice.idInvoice = invoiceAdd.invoiceId)invoice" +
                    ...
                    " from(select org.firmName,loadingSlipId,person.firstName as firstName, person.lastName as lastName, invoiceDate, idInvoice, statusDate, invoiceNo, deliveryLocation, discountAmt, cgstAmt, sgstAmt, igstAmt, grandTotal, vehicleNo, invoice.createdOn, isConfirmed, statusId, narration,invFromOrgId from finalInvoice invoice  LEFT JOIN tblOrganization org on org.idOrganization = invoice.distributorOrgId Left Join tblPerson person on person.idPerson = org.firstOwnerPersonId INNER JOIN finalInvoiceAddress invoiceAdd on invoice.idInvoice = invoiceAdd.invoiceId)invoice" +
